...
async fn search_docs(
    ...
    let hyde = HypotheticalDoc::new(local_client, embedder, ann_index, 1000);
    ...
    match hits_result {
        Ok(hits) => {
            // hits is HydeResponse, containing the stream
            print_query_results(hits).await?;
            println!(
                "Answer generated by {} in {:.2?}:",
                hyde_model_name, duration
            );
            Ok(())
        }
        Err(e) => {
